...
test('getProgressSurfaceModel derives athlete metrics from completed sessions', () => {
  ...
  assert.equal(model.readinessInterpretation.title, 'Readiness interpretation')
  assert.match(model.readinessInterpretation.body, /2 completed sessions are driving a moderate readiness signal/i)
  assert.equal(model.readinessInterpretation.rows.length, 3)
  assert.equal(model.readinessInterpretation.rows[0].title, 'Recent load')
  assert.match(model.readinessInterpretation.rows[0].body, /Weekly load score is 132/i)
  assert.equal(model.readinessInterpretation.rows[1].title, 'Lower-body fatigue')
  assert.match(model.readinessInterpretation.rows[1].body, /Quads: high fatigue/i)
  assert.equal(model.readinessInterpretation.rows[2].title, 'Coach read')
  assert.match(model.readinessInterpretation.rows[2].body, /push quality work, but recovery is no longer wide open/i)
...
test('getProgressSurfaceModel falls back cleanly when no completed sessions exist', () => {
  ...
  assert.equal(model.readinessInterpretation.title, 'Readiness interpretation')
  assert.match(model.readinessInterpretation.body, /No completed sessions are feeding readiness yet/i)
  assert.equal(model.readinessInterpretation.rows.length, 3)
  assert.match(model.readinessInterpretation.rows[0].body, /No completed load yet/i)
  assert.match(model.readinessInterpretation.rows[2].body, /Finish a session to turn readiness into a real signal/i)
  assert.equal(model.recentMomentum.title, 'Recent momentum')
