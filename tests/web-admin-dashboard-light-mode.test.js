...
test('admin dashboard overview body uses light-mode theme tokens for cards charts and controls', () => {
  ...
  assert.match(selectSource, /data-\[highlighted\]:text-\[var\(--admin-shell-accent\)\]/)
  assert.match(selectSource, /bg-\[var\(--admin-dashboard-card-bg\)\]/)
  assert.doesNotMatch(selectSource, /border-\[#24334A\]|bg-\[#111D30\]|bg-\[#F8FAFC\]|text-\[#DCE6F8\]|focus:bg-\[#15233A\]/)
  ...
  assert.match(dashboardSource, /SelectTrigger[\s\S]*className="admin-shell-overview-select-trigger hidden w-\[160px\] sm:ml-auto sm:flex"[\s\S]*aria-label="Select training execution range"/)
  assert.match(cssSource, /\.admin-shell-overview-select-trigger\s*\{[^}]*border:\s*1px solid var\(--admin-dashboard-card-border\) !important;[^}]*background:\s*var\(--admin-dashboard-control-bg\) !important;[^}]*color:\s*var\(--admin-dashboard-card-text\) !important;/)
  assert.match(cssSource, /\.admin-shell-overview-select-trigger:hover\s*\{[^}]*background:\s*var\(--admin-dashboard-control-hover-bg\) !important;/)
  assert.match(cssSource, /\.admin-shell-overview-select-trigger:focus-visible\s*\{[^}]*border-color:\s*var\(--admin-shell-accent\) !important;[^}]*box-shadow:\s*0 0 0 2px var\(--admin-input-focus\) !important;/)
  assert.match(cssSource, /\.admin-shell-overview-select-content\s*\{[^}]*border:\s*1px solid var\(--admin-dashboard-card-border\) !important;[^}]*background:\s*var\(--admin-dashboard-card-bg\) !important;[^}]*color:\s*var\(--admin-dashboard-card-text\) !important;/)
  assert.match(dashboardSource, /admin-shell-overview-card-value/)
  assert.doesNotMatch(dashboardSource, /needsAttention:\s*AlertTriangle/)
  ...
  assert.match(dashboardSource, /workoutResultsChartConfig/)
  assert.match(dashboardSource, /Training consistency/)
  ...
  assert.match(cssSource, /\.admin-shell-overview-performance-panel\s*\{[^}]*border-radius:\s*var\(--radius-xl\);/)
  assert.match(cssSource, /\.admin-shell-overview-insight-card\s*\{[^}]*border-radius:\s*var\(--radius-xl\);/)
