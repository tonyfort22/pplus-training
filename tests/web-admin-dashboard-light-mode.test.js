...
test('admin dashboard overview body uses light-mode theme tokens for cards charts and controls', () => {
  ...
  assert.match(dashboardSource, /admin-shell-overview-mini-bar-completed/)
  assert.match(dashboardSource, /admin-shell-overview-mini-bar-assigned/)
  ...
  assert.match(cssSource, /\.admin-shell-overview-performance-chart\s*\{[^}]*grid-template-columns:\s*repeat\(auto-fit, minmax\(42px, 1fr\)\);/)
  assert.match(cssSource, /\.admin-shell-overview-mini-bar-group\s*\{[^}]*display:\s*grid;/)
