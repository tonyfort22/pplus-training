...
test('getCompletedSessionSurfaceModel builds a post-workout summary from actual completed work', () => {
  ...
  assert.equal(model.highlights.title, 'Session recap')
  assert.equal(model.highlights.rows[0].title, 'Outcome')
  assert.match(model.highlights.rows[0].body, /feeds progress/)
  assert.equal(model.highlights.rows[1].title, 'Work completed')
  assert.match(model.highlights.rows[1].body, /7\/7 sets/)
  assert.equal(model.highlights.rows[2].title, 'Strongest set')
  assert.match(model.highlights.rows[2].body, /Barbell Back Squat/)
  assert.match(model.highlights.rows[2].body, /120 lb x 8/)
...
test('getTrainRenderModel converts a completed-session surface into summary sections', () => {
  ...
  assert.equal(renderModel.content.sections[2].title, 'Session recap')
  assert.equal(renderModel.content.sections[3].type, 'body-list')
  assert.equal(renderModel.content.sections[3].title, 'Logged exercises')
