...
test('training builder seeds only the first section and first exercise expanded on load', () => {
  const workoutTrainingBuilderSource = readFileSync(workoutTrainingBuilderPath, 'utf8')

  assert.match(workoutTrainingBuilderSource, /function createExercise\(id, title, defaultSets = \[], overrides = \{\}\) \{[\s\S]*isExpanded:\s*overrides\.isExpanded \?\? true,/)
  assert.match(workoutTrainingBuilderSource, /createSection\('A1',[\s\S]*isExpanded:\s*true/)
  assert.match(workoutTrainingBuilderSource, /createSection\('A2',[\s\S]*isExpanded:\s*false/)
  assert.match(workoutTrainingBuilderSource, /createExercise\('exercise-skater-hops', 'Skater hops', EXERCISE_LIBRARY\[0\]\.defaultSets, \{ isExpanded: true \}\)/)
  ...
  assert.match(workoutTrainingBuilderSource, /createExercise\('exercise-copenhagen-plank', 'Copenhagen plank', EXERCISE_LIBRARY\[2\]\.defaultSets, \{ isExpanded: true \}\)/)
