...
test('admin settings account uses light-mode admin tokens instead of dark-coded field colors', () => {
  ...
  assert.match(accountSource, /className=\{`admin-settings-account-email \$\{settingsFieldInputClassName\}`\}/)
  assert.match(accountSource, /className=\{`admin-settings-account-current-password \$\{settingsFieldInputClassName\}`\}/)
  assert.match(accountSource, /className=\{`admin-settings-account-new-password \$\{settingsFieldInputClassName\}`\}/)
  assert.match(accountSource, /className=\{`admin-settings-account-confirm-password \$\{settingsFieldInputClassName\}`\}/)
  assert.match(accountSource, /border border-\[var\(--admin-shell-accent\)\]\/30 bg-\[var\(--admin-shell-accent\)\]\/10/)
  assert.match(accountSource, /bg-\[var\(--admin-shell-primary-button-bg\)\] text-\[#0B1120\]/)
