...
test('assignWorkoutTemplatesToProgram skips archived templates instead of creating planned workouts from stale selections', async () => {
  const calls = []
  const templates = {
    'active-template': { id: 'active-template', coach_id: 'coach-1', name: 'Active speed', training_type: 'Speed', status: 'active', bg_color: '#101828', text_color: '#ffffff' },
    'archived-template': { id: 'archived-template', coach_id: 'coach-1', name: 'Archived lift', training_type: 'Strength', status: 'archived', bg_color: '#111827', text_color: '#ffffff' },
  }
  const fetchImpl = async (url, options = {}) => {
    const parsedUrl = new URL(url)
    const table = parsedUrl.pathname.split('/').pop()
    ...
    if (table === 'workout_templates' && method === 'GET') {
      ...
      if (idFilter.startsWith('in.')) {
        return createJsonResponse(Object.values(templates))
      }

      const id = idFilter.replace('eq.', '')
      return createJsonResponse(templates[id] ? [templates[id]] : [])
    }
    ...
    if (table === 'workout_template_sets' && method === 'GET') {
      return createJsonResponse([{ id: 'template-set-1', workout_template_exercise_id: 'active-template-exercise-1', sort_order: 0, set_type: 'straight', target_reps: 8, target_load: null, target_load_unit: null, target_duration_seconds: null, target_distance: null, target_distance_unit: null, target_rpe: null, target_rir: null, target_rest_seconds: 45, notes: null }])
    }
    ...
    if (table === 'program_workouts' && method === 'GET') {
      const id = parsedUrl.searchParams.get('id')?.replace('eq.', '')
      return createJsonResponse([{ id, program_id: 'program-1', athlete_id: 'athlete-1', coach_id: 'coach-1', workout_template_id: 'active-template', name_snapshot: 'Active speed', status: 'scheduled', sort_order: null }])
    }
  ...
  const result = await repository.assignWorkoutTemplatesToProgram({
    programId: 'program-1',
    workoutTemplateIds: ['active-template', 'archived-template', 'active-template'],
  })

  const programWorkoutPosts = calls.filter((call) => call.table === 'program_workouts' && call.method === 'POST')
  assert.equal(programWorkoutPosts.length, 1)
  assert.equal(programWorkoutPosts[0].body.workout_template_id, 'active-template')
  assert.deepEqual(result.assignedWorkouts.map((assignedWorkout) => assignedWorkout.workout.id), ['program-workout-1'])
  assert.deepEqual(result.skippedWorkouts, [{ id: 'archived-template', name: 'Archived lift', status: 'archived', reason: 'Archived workout templates cannot be assigned to a program.' }])
})
