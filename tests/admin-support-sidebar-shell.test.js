...
test('support inbox shell uses a support-specific resizable sidebar around the chat reference', () => {
  ...
  assert.match(source, /className="min-w-0 flex-1 bg-\[#070D18\]"/, 'chat section should be pure flex-1 and fill remaining width')
...
test('support conversation sidebar keeps one brand logo, trigger behavior, and no leaked admin dropdown carets', () => {
  ...
  assert.equal(logoAssetMatches.length, 1, 'support sidebar should use exactly one PPLUS training logo asset')
  assert.equal(imageTagMatches.length, 1, 'support sidebar should render exactly one logo img source, not expanded/collapsed duplicates')
  assert.match(source, /useSidebar/, 'support sidebar should read sidebar state for collapsed-safe logo rendering')
  assert.match(source, /!isCollapsed \? <img/, 'support sidebar should unmount the full logo while collapsed instead of clipping it')
  ...
  assert.match(source, /className="h-9 w-full border-white\/10/, 'support search input should span the full sidebar width')
  assert.match(source, /<Search className="pointer-events-none absolute left-3/, 'support search icon should sit inside the full-width input')
...
test('support conversation rows use a clean non-wrapping responsive card layout', () => {
  ...
  assert.match(source, /className="min-w-0 flex-1 truncate/, 'conversation name should truncate cleanly')
  assert.match(source, /ItemTitle className="min-w-0 w-full overflow-hidden text-\[13px\] text-white"/, 'conversation subject row should own the constrained width')
  ...
  assert.match(source, /ItemDescription className="truncate/, 'conversation preview should be one-line truncated')
...
test('collapsed support conversation rows keep tappable avatar icons visible', () => {
  ...
  assert.match(source, /Avatar className="h-8 w-8[^"]*group-data-\[collapsible=icon\]:h-8 group-data-\[collapsible=icon\]:w-8/, 'collapsed support rows should keep the conversation avatar visible')
